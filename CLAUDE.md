...
| `WAV_CACHE_MAX_MB` | 任意 | デフォルト64。VOICEVOX/Fish それぞれのWAVキャッシュに掛かる合計バイト上限 (MB) |
...
├── userDict.js       # VOICEVOXユーザー辞書の更新 (Engine と JSON の整合・直列化)
...
└── commands/         # スラッシュコマンド群 (トップレベルは join/leave/skip/voice/dict/config の6つ)
    ├── index.js      # コマンドを Map にまとめ、Guild限定の付与と実行時ガードを一括で掛ける
    ├── i18n.js       # setNameLocalizations 用の ja ヘルパー
    ├── join.js / leave.js
    ├── skip.js       # 再生中の読み上げをスキップ (all:true でキューも全消し)
    ├── voice.js      # /voice set|show|reset (個人の話者・速度・pitch・intonation・Fishボイス/感情)
    ├── voiceList.js  # /voice list (VOICEVOX話者 + Fishボイスの一覧)
    ├── fishAdmin.js  # /voice fish add|remove (removeは要ManageGuild)
    ├── config.js     # /config show|set|channels|reset (ギルド単位の読み上げ挙動設定)
    ├── ignoreSettings.js # /config ignore user|prefix|bots|me|list (読み上げ除外)
...
- **話者の優先順位**: 個人設定 (`/voice` の `speaker`) > 凍結済みの自動割り当て (`autoSpeaker`) > `userId % 話者数` による決定論的割り当て > ギルドデフォルト (speaker=3)。**自動割り当ての結果は初回に `userSettings.json` の `autoSpeaker` へ焼き付ける** — `userId % ids.length` は engine のスタイル数と並び順に依存するので、VOICEVOX を上げてスタイルが1つ増減しただけで `/voice` 未設定ユーザーの声が全員入れ替わる (「同じ人は常に同じ声」が壊れる)。凍結値は `speaker` とは別のキーに持つ (混ぜると `/voice show` が自動割り当てを「設定済み」と表示する)。凍結した ID が engine から消えていたら割り当て直して凍結し直し、**Engine 不通でギルド既定話者へフォールバックしたときは凍結しない** (焼き付けると復旧後も全員が既定話者のままになる)。保存の失敗は読み上げを止めず、ログに残して次の発言で再試行する。速度・pitch・intonation は個人設定がなければ既定値 (1.0 / 0.0 / 1.0)。`userVoice.resolveUserVoice()` が `{ engine, speaker, fishRef, fishEmotion, speed, pitch, intonation }` を返し、`tts.synthVoice()` がエンジンを選んで合成する。**engine が `fish` でも VOICEVOX の `speaker` は必ず解決する** — Fish の日次バイト上限超過時のフォールバック先として必要なため。**明示指定の話者IDも合成前に存在確認する**: 存在しない ID だと合成が毎回 422 で落ち、そのユーザーの発言が永久に無音になるため、既定話者へ倒して読み上げを継続し、直し方をユーザーごと一度だけログに出す。ただし**話者一覧を取得できないときは明示指定を尊重する** (Engine 不通のたびに全ユーザーの声を既定話者へ倒すと総入れ替わりになる)。`/voice set speaker:` 側も一覧を取れなければ**操作全体を拒否する** — 検証できないまま保存すると復旧後も無音が続き、speaker だけ捨てて speed 等を保存すると「一部だけ効いた」状態になるため
- **TTSエンジンの二本立て**: 既定は VOICEVOX。`/voice set fish:<エイリアス|reference_id>` を明示指定したユーザーだけが Fish Audio ([fish.audio](https://fish.audio)) を使う。`/voice set speaker:` と `/voice set fish:` は排他 (同時指定はエラー)。**自動ランダム割り当ての対象は VOICEVOX 話者のみ**で Fish は混ぜない (課金と ID 空間が別物のため)。`speed` のレンジは両者 0.5〜2.0 で一致する。`pitch` は VOICEVOX 固有 (`audio_query` のフィールド) で Fish に対応物が無いため無視されるが、`intonation` は `fishAudio.intonationToTemperature()` で Fish の `temperature` に換算して効かせる。`synth()` の呼び出し口を `tts.synthVoice()` の1箇所に集約してあるので、エンジン追加時に `player.js` を触る必要はない
- **Fish の抑揚と感情タグ**: `/voice set intonation` (0.0〜2.0) を Fish の `temperature` (0.1〜1.0) へ**折れ線**で換算する (`0→0.1` / `1→0.7` / `2→1.0`)。単純な線形換算だと既定値 1.0 が 0.55 になり、何も設定していない既存ユーザーの声が黙って平坦になるため、VOICEVOX 既定の 1.0 が Fish 既定の 0.7 に一致する形にしてある (未指定時は `temperature` フィールド自体を送らない)。`top_p` は触らない。`/voice set fish_emotion` は本文頭に `[happy]` のようなタグを付けて話し方を変えるもので、**S2 系モデル限定** (`s1` に付けると literal に読まれるため `supportsEmotion()` で判定してスキップ)。タグは choices 固定 — 自由入力だとタグとして解釈されない文字列がそのまま読み上げられ、しかも課金バイトに乗る。タグの付与は `fishAudio.applyEmotion()` を `tts.js` から呼ぶ形にしてある: `synth()` の中で付けると課金バイトをタグ抜きで数えて日次上限の判定がずれるため。**VOICEVOX へのフォールバック時はタグなしの本文を使う** (VOICEVOX は `[happy]` をそのまま読む)
...
- **コマンドは4系統に畳んである**: トップレベルは `/join` `/leave` (VCの出入り)・`/voice` (自分の声)・`/dict` (読み方)・`/config` (サーバーの挙動) だけ。旧 `/speakers` は `/voice list`、旧 `/fishvoice` は `/voice fish add|remove`、旧 `/ignore` は `/config ignore …` へ統合した。**単独では意味を持たないコマンドをトップレベルに置かない** — `/speakers` も `/fishvoice list` も「`/voice` に何を入れればいいか」を調べるためだけの口で、実際 `/voice` の説明文が他コマンドへの参照 (「/speakers で一覧を確認」) で成り立っていた。Fishボイスの一覧は `/voice list` の1箇所だけが持つ (`/voice fish list` を残すと同じ内容が2箇所に出る)。統合先が肥大しないよう、実装は `voiceList.js` / `fishAdmin.js` / `ignoreSettings.js` にサブコマンド定義ごと切り出し、親 (`voice.js` / `config.js`) は `addSubcommand(listSubcommand)` のように組み立てて振り分けるだけにしてある
- **オプション名は API 名を ASCII のまま日本語表示にする**: `setNameLocalizations({ ja: ... })` (`commands/i18n.js` の `ja()`) で `ja` ロケールの表示名だけ差し替える。API 名を日本語にすると `interaction.options.getInteger("max_length")` 側まで巻き込むため触らない。**トップレベルのコマンド名だけは敢えてローカライズしない** — サブコマンドとオプションはピッカーから選ぶが、コマンド名は利用者が直接打ち込むので、日本語にすると IME の切り替えを強いることになる。ローカライズ名も Discord の名前規則に従う (空白・記号は不可、ラテン文字は小文字のみ)。付け忘れは `test/commandStructure.test.js` が落とす
...
- **音声キュー**: guild ごとに `player.js` の `sessions` Map で管理。`MAX_QUEUE` (100) を超える enqueue は破棄 (連投対策)。`skip(guildId, all)` で再生中をスキップ (`all=true` でキュー全消し)。合成/再生に失敗した1件は `drain()` がログを出してスキップし、キュー全体は止めない。話者解決 (`resolveUserVoice()`) は `enqueue()` 時点ではなく `drain()` の中、合成の直前で行う。`enqueue()` は外部通信を待たない同期処理なので、話者一覧取得を待っている間に後続の発言が先にキューへ積まれて発言順が逆転することがなく、キュー上限の判定も外部通信より先に効く。`/skip` でも同じ `skip()` を呼べる (連投を止めるのに `/leave` して読み上げ対象chの設定ごと失う必要がない)。認可は `authorizeSkip()` — Bot と同じ VC にいる人、または `ManageGuild`/`MoveMembers` を持つ人に限る (VC外の人が読み上げを妨害できないようにするため、`authorizeLeave` と同じ考え方)
- **再生リソースの確実な破棄**: `createAudioPlayer()` は `noSubscriber: Stop` で作る (既定の Pause のままだと、退出/VC移動で購読者を失った player が古い resource と ffmpeg プロセスを掴んだまま残る)。退出・VC移動・session 差し替えのたびに `stopCurrentAudio()`(`player.stop(true)` + 生きている ffmpeg の kill) を必ず通し、`discardSession()` でキューごと畳む。`drain()` は `synthVoice()`/`resolveUserVoice()` の await 後に `sessions.get(guildId)` がまだ同じ session かを確認してから再生資源を作る (合成待ちの間に session が捨てられていたら、古い世代の音声を新しい session や誰も購読していない player に流さない)
- **Ready へ戻らない接続のウォッチドッグ**: close code 4006/1006 では @discordjs/voice が `Disconnected` を経由せず `Signalling` へ直接遷移して join payload を再送するだけになり、そのまま `Ready` に戻らず滞留しても検知する仕組みが無い。`connection` の `stateChange` を見て `Ready` 以外が30秒続いたら見切って破棄し、あとは `VoiceStateUpdate` の自動参加に復帰を任せる (数秒で戻る通常の再接続はしきい値に達しないので影響しない)
...
- **VOICEVOX クライアントの耐性**: `voicevox.js` の `request()` は一時的な不通/5xxに対して200ms→400msのバックオフで最大2回リトライする (4xxは即諦める)。全リクエストに 30 秒のタイムアウトを掛ける (`AbortSignal.timeout`)。タイムアウト時はリトライしない — Engine が応答不能な状態で再挑戦してもキューを止める時間が延びるだけのため。`/user_dict_word` の POST は冪等でないので `retry: false` (5xx を返しつつ登録済みだと別 uuid で二重登録されるため)。`synth()` は `speaker:speed:pitch:intonation:text` をキーにした挿入順Map (上限100件、簡易LRU) でWAVをキャッシュし、同一文言の再合成コストを削減する。**キーに辞書の内容は入らないため、ユーザー辞書を変えたら世代 (`bumpUserDictRevision()`) を上げてキャッシュを捨てる** — でないと、変更前に一度読ませた文は LRU から追い出されるか Bot 再起動まで旧発音のまま返り、頻出文ほど辞書の修正が効かない。世代は Engine 側の辞書の状態を追うので、Engine を変更する関数 (add/update/delete/import) の成功直後に上げる。`clear()` だけでは足りず、`synth()` は**合成開始時点の世代を控えて、完了時に変わっていたらキャッシュに載せない** (変更の前に始まった合成が変更後に完了し、旧発音の WAV を新世代へ入れ直す race が残るため)。非圧縮WAVは長文・低速ほど大きくなるため、件数(100)に加えて合計バイト数 (既定64MB、`WAV_CACHE_MAX_MB`) でも頭打ちにし、単一エントリが予算の1/8を超えるものはキャッシュに載せない (Fish Audio 側の WAV キャッシュにも同じ上限を適用)。`getSpeakerIds()` は cache miss 中の同時呼び出しを1つの fetch に相乗りさせ (single-flight)、失敗後は10秒のクールダウンを置いてリトライ嵐を防ぐ。失敗時はギルドデフォルト話者にフォールバック。起動時に疎通確認の警告ログを出すが Bot は停止しない
- **ログに読み上げ本文を残さない**: 読み上げ本文は `/audio_query` の `text`、ユーザー辞書の語は `/user_dict_word` の `surface`/`pronunciation` として**クエリ文字列に乗る**。エラーメッセージにパスをそのまま埋めると、Engine が一時的に落ちるたびに Discord の発言本文が URL エンコードされた形でコンテナの標準出力に残り続ける (`log.js` も `store.js` も本文を残さない設計なので、ここだけ抜けているのは筋が悪い)。`safePath()` が値を `<redacted:NN文字>` に置き換え、`speaker`/`accent_type`/`priority`/`override` のような非機密パラメータだけ残す。**FastAPI の 422 はリクエスト内容をそのまま返す**ため、本文由来のデータを送るリクエスト (`/audio_query`・`/user_dict_word`・`sensitiveBody` を立てた `/synthesis`・`/import_user_dict`) は応答本文も伏せる。調査時は `DEBUG_LOG_TEXT=1` で生のパスと応答本文を出せる。**入力エラーと障害の区別**には `request()` が投げる `VoicevoxHttpError` (`status` を持つ) を使い、`isInputError()` が 4xx を入力エラー、timeout/接続失敗/5xx を Engine 障害として分ける (混ぜると、直せる入力ミスを「接続できない」と誤診する)。ユーザーへ返す説明は `extractValidationMessage()` が pydantic の detail から `Value error,` と `[type=` に挟まれた**固定文言だけ**を取り出したもので、`input_value`(=辞書の語そのもの) は捨てる — `console.error(err)` はカスタムプロパティも展開して出力するため、未知の本文を `detail` に載せるとそこから漏れる
- **辞書は2系統のハイブリッド構成**: `/dict replace` (既存の文字列置換辞書、`data/dictionary.json`、guildId単位) はどんな表層形/読みでも登録できる。**全エントリを1本の正規表現にまとめた1パス同時置換**で、word は長い順に並べて最長一致を担保し、正規表現エスケープしてリテラル一致させる (`(^^)` のような記号エントリがあるため)。逐次置換だと reading が別エントリの word を含んだときに置換結果が再度置換され、登録順で結果が変わるうえ、連鎖すると中間文字列が指数的に膨らんでプロセスごと落ちる。保険として置換後 4000 文字で頭打ちにし、ギルドあたり 200 件の登録上限も設けている。`/dict word` (VOICEVOXユーザー辞書、`data/userDict.json`、全サーバー共通) は VOICEVOX の形態素解析に単語として正式登録するため読み精度が上がるが、`pronunciation` は全角カタカナのみ受け付ける (`voicevox.js` の `hiraganaToKatakana` でひらがな入力を変換し、`validatePronunciation()` で検証)。**検証規則は VOICEVOX 0.25.2 の `user_dict/model.py` を移植したもの** — 以前の `/^[゠-ヿ]+$/` は `ア・イ` や `ヵ`/`ヶ` まで通し、Engine の `[ァ-ヴー]` で 422 になっていた。捨て仮名の連続と「くゎ」「ぐゎ」以外の「ゎ」も同じ規則で弾く。アクセント型の上限はモーラ数 (`countMora()`、`text_utility.py` の移植) で決まり slash command の静的な `setMaxValue` では表せないため実行時に見る。ローカル検証は往復を省いて具体的に案内するためのもので、**正は常に Engine の 422 側**に置く (規則が食い違っても Engine が最後に弾く)。既存の顔文字/記号系エントリ (`(^^)` 等) はカタカナ化できないため `dictionary.json` 側に残したまま移行していない。`voicevox_engine` コンテナは user_dict を永続化しないため、`data/userDict.json` の内容 (登録時にVOICEVOXが発行したuuidを保持) を engine へ再投入して復元する
- **辞書の復元は起動時1回ではなく定期的な突き合わせで行う**: `userDict.js` の `startUserDictSync()` を `ClientReady` で開始し、60秒ごとに `reconcileUserDict()` が `GET /user_dict` と `data/userDict.json` を比べる。**`isAlive()` を条件にしない** — cold start で engine の起動が Bot より遅れただけで辞書が Bot の再起動まで戻らず、運用中に engine だけ再作成した場合も同じことが起きる (`depends_on` はコンテナの起動順しか保証しない)。判定は **保存済みの uuid が engine に在るか**だけを見る: engine には再起動を示すマーカーが無いのでこれが唯一の確実な検知手段で、逆に engine 側にだけ在る語や内容の差分を mismatch に含めると毎回 import し続ける無限ループになる (`import_user_dict` は既存語を消さず、`surface` は engine 側で半角英字が全角へ正規化されるため)。失敗時は 5s→300s のバックオフで再試行し、**警告ログは連続失敗の初回だけ**出す。timer は `unref()` してプロセスの終了を妨げない。辞書が0件なら engine を叩かない。compose 側にも `healthcheck` + `condition: service_healthy` を入れてあるが、これは初回の race を減らすだけの補助で、正はこのアプリ側の再同期
- **ユーザー辞書の更新は `userDict.js` に集約し、Engine と JSON の整合を守る**: 守れないと何が起きるかが非対称で、JSON にしか無い語は次の起動時の復元で Engine へ戻るのに対し、**Engine にしか無い語は uuid を知る手段が無くなって二度と消せない孤児**になる (単一 Engine なので全ギルドの発音に効き続ける)。そのため (1) 既存語の更新は `PUT /user_dict_word/{uuid}` を使う — 旧実装の「DELETE してから POST」は DELETE 成功+POST 失敗で語が消え、DELETE 失敗 (握り潰していた) +POST 成功で重複が残った。(2) **必ず Engine 側の成功を確認してから JSON へ commit する** (remove も同様に DELETE が先。失敗時は JSON を消さず成功と偽らない → 再実行で消せる)。(3) JSON の保存に失敗したら Engine をロールバックし、それも失敗したら **uuid をログに残す** (手で消すのに要る唯一の情報)。(4) 辞書操作は1本の Promise チェーンへ直列化する — 同じ語への add と remove が交差すると、片方だけ進んだ状態が残るため。`store.js` の `addUserDictEntry`/`removeUserDictEntry` も **save() が成功してから in-memory を差し替える** (逆順だと保存失敗時に「メモリ上は登録済み・ディスクには無い」語が生まれ、次の起動で復元対象から漏れて孤児になる)。DELETE の 404 は「既に消えている」として成功扱い、PUT の 404 だけは新規 POST へ倒す (Engine 再作成後の復帰)
...
- **engine イメージはタグ固定**: `docker-compose.yml` は `voicevox/voicevox_engine:cpu-0.25.2` に固定してある。バージョンを上げるときは、上げる前に `curl -s localhost:50021/speakers | jq '[.[].styles[].id]'` を記録し、更新後に件数と並び順を比較する。一度でも発言したユーザーは自動割り当てが `autoSpeaker` に凍結済みなので声は変わらないが、**まだ一度も発言していないユーザーは更新後の一覧で割り当てられる**。廃止されたスタイルIDを保存済みのユーザーは、自動割り当てなら割り当て直し、`/voice` で明示指定していれば既定話者へフォールバックする（起動後のログに警告が出る）。
...
- **ログは 10MB × 3 世代でローテート**: 両サービスに `logging` を設定済み。`logging` は再作成が必要な項目なので、変更を反映するには `docker compose up -d` でコンテナを作り直す（engine を作り直すと VOICEVOX 側の user_dict は消えるが、Bot の定期同期が最大60秒で入れ直すので Bot 側の再起動は不要）。

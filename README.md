...
- `/voice set|show|reset` … 自分の話者・速度・声の高さ・抑揚・Fishの感情を設定（全サーバー共通）
- `/voice list` … 使える声（VOICEVOX話者 + Fishボイス）の一覧
- `/voice fish add|remove` … Fish Audio ボイスの登録・削除
- `/dict replace|word` … ローカル置換辞書・VOICEVOXユーザー辞書の管理
- `/config show|set|channels|reset` … 対象チャンネル、VC入退室通知、発言者名、最大文字数の設定
- `/config ignore` … ユーザー・プレフィックス・Bot発言・自分の読み上げ除外
...
チャンネルの発言を読み上げる。`/voice set` で自分の声、`/config set` でサーバーの
読み上げ挙動、`/config ignore` で除外条件を変更できる（使える声は `/voice list` で確認）。

コマンド名は英語のままだが、サブコマンド名とオプション名には日本語のローカライズ名を
付けてあるので、Discord の表示言語が日本語なら入力欄のラベルは日本語で出る。
